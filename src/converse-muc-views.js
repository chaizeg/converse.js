    ...
    initialize () {
        ...
        _converse.ListChatRoomsModal = _converse.BootstrapModal.extend({
            ...
            roomStanzaItemToHTMLElement (groupchat) {
                ...
                div.innerHTML = tpl_room_item({
                    ...
                    // 'project': this.defineProject(groupchat),
            ...
            //added 
            defineProject(groupchat){
                var member;
                var str = _converse.bare_jid;
                for (var i = 0; i < groupchat.get('occupants').length; i++){
                    member = this.model.occupants[i];
                    var domainJid = str.substring(
                        str.indexOf("@") + 1, 
                        str.indexOf("/") != - 1? str.indexOf("/"):str.length);
                    var domainGC = member.getAttribute('jid').substring(member.getAttribute('jid').indexOf(".")+1);
                    if(domainJid != domainGC){
                        console.log((externeeee));
                        return 'external';
                    }
                }
                if(!groupchat.get('occupants') || groupchat.get('occupants').length == 0){
                    return 'pbm';
                }
                return 'internal';
            },
            ...
            onRoomsFound (iq) {
                 ...
                 */
                console.log('handle ingoing room info');
        ...
        _converse.ChatRoomView = _converse.ChatBoxView.extend({
            ...
            defineProject(){
                var member;
                var str = _converse.bare_jid;
                for (var i = 0; i < this.model.occupants.length; i++){
                    member = this.model.occupants[i];
                    var domainJid = str.substring(
                        str.indexOf("@") + 1, 
                        str.indexOf("/") != - 1? str.indexOf("/"):str.length);
        ...
        _converse.MUCNicknameForm = Backbone.VDOMView.extend({
            ...
            toHTML () {
                ...
                return tpl_chatroom_nickname_form({
                    ...
                    'label_join': __('Enter groupchat test'),
                    'error_class': err_msg ? 'error' : '',
                    'validation_message': err_msg,
